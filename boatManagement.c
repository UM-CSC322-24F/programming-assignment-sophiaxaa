...
typedef union {
    ...
    char trailorTag[20];  // the trailor license tag
...
void loadData(const char* filename, Boat* boats[]) {
    ...
    while (fgets(line, sizeof(line), file) && index < MAX_BOATS) {
        ...
        char type[20];
        sscanf(line, "%[^,],%f,%[^,],%[^,],%f", boat->name, &boat->length, type, boat->info.trailorTag, &boat->amountOwed);

        boat->type = getBoatType(type);
        switch (boat->type) {
            case SLIP:
                sscanf(boat->info.trailorTag, "%d", &boat->info.slipNum);
                break;
            case LAND:
                boat->info.bayLetter = boat->info.trailorTag[0];
                break;
            case TRAILOR:
                // trailor tag is already set above
                break;
            case STORAGE:
                sscanf(boat->info.trailorTag, "%d", &boat->info.storageNumber);
                break;
            default:
                break;
        }
        ...
        if (boats[i]) {
            ...
            switch (boats[i]->type) {
                ...
                case LAND:
                    ...
                    break;
                case TRAILOR:
                    fprintf(file, "%s,", boats[i]->info.trailorTag);
                    break;
                case STORAGE:
...
//sorted alphabeticallly by name of boat using qsort
void printInventory(Boat* boats[]) {
        ...
        if (boats[i]) {
            ...
            switch (boats[i]->type) {
                ...
                case LAND:
                    ...
                    break;
                case TRAILOR:
                    printf("%-8s ", boats[i]->info.trailorTag);
                    break;
                case STORAGE:
...
void addBoat(const char* csvData, Boat* boats[]) {
    ...
    char type[20];
    sscanf(csvData, "%[^,],%f,%[^,],%[^,],%f", boat->name, &boat->length, type, boat->info.trailorTag, &boat->amountOwed);
...
// accept boat payment - account for more than amount owed
void acceptPayment(const char* name, Boat* boats[]) {
    for (int i = 0; i < MAX_BOATS; i++) {
        if (boats[i] && strcasecmp(boats[i]->name, name) == 0) {
            float amount;
...
//update amount owed for a new month per boat
void updateAmountOwed(Boat* boats[]) {
    for (int i = 0; i < MAX_BOATS; i++) {
        if (boats[i]) {
            float rate;
...
BoatType getBoatType(const char* typeStr) {
    ...
    if (strcmp(typeStr, "trailor") == 0) return TRAILOR;
